...
void	vic_advance_raster(_bus *bus, _VIC_II *vic, unsigned cpu_cycles) {
	...
	for (; vic->cycles >= 63; vic->cycles -= 63) {
		...
		visible_screen = (control1 >> 0x4) & 0x1;
		bitmap = (control1 >> 0x5) & 0x1;
		extended = (control1 >> 0x6) & 0x1;
		multicolor = (control2 >> 0x4) & 0x1;

		if (!visible_screen || vic->raster < 56 || vic->raster >= 256) {
			for (unsigned x = 0; x < 504; x++)
		...
		if (vic->raster == vic->get_raster(vic)) {
			...
			bus->ram_write(bus, INTR_STATUS, _D019 & ~0x1);
